...
import { Search, Plus, Eye, Pencil, Trash2, Package, Upload, UserPlus, UserMinus, X, SlidersHorizontal, RotateCcw, MoreVertical } from 'lucide-react';
            ...
            <>
              ...
              <div className="block md:hidden space-y-2.5">
                {filtered.map((a) => (
                  ...
                  >
                    {/* 1. Top Row: Asset Name + In Use Status Badge & 3-Dot Action Menu (Asset Code on next line) */}
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <h4 className="text-[13.5px] font-bold text-slate-900 leading-snug tracking-tight" title={a.name}>
                            {a.name}
                          </h4>
                          <StatusBadge status={a.status} className="!text-[10px] !px-1.5 !py-0.5 shrink-0" />
                        </div>
                        <div className="mt-1">
                      ...
                      <div className="relative shrink-0">
                        <button
                          type="button"
                          onClick={() => setActiveActionMenuId(activeActionMenuId === a.id ? null : a.id)}
                        ...
                        {activeActionMenuId === a.id && (
                          <>
                            <div
                              className="fixed inset-0 z-40"
                              onClick={() => setActiveActionMenuId(null)}
                            />
                            <div className="absolute right-0 top-full mt-1.5 w-40 bg-white border border-slate-200 rounded-xl shadow-xl z-50 py-1 overflow-hidden animate-in fade-in zoom-in-95 duration-100">
                              <button
                                type="button"
                              ...
                              {canAllocate && a.status === 'available' && (
                                <button
                                  type="button"
                                  className="w-full px-3 py-2 text-left text-xs font-medium text-emerald-700 hover:bg-emerald-50 flex items-center gap-2 cursor-pointer border-none bg-transparent transition-colors"
                                  onClick={() => {
                                    setActiveActionMenuId(null);
                                    openAllocate(a);
                                  }}
                                >
                                  <UserPlus size={13.5} className="text-emerald-600" />
                                  <span>{t('allocate') || 'Allocate'}</span>
                                </button>
                              )}
                              ...
                              {canReturn && a.status === 'allocated' && (
                                ...
                                >
                                  <UserMinus size={13.5} className="text-amber-600" />
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-start pb-3 border-b border-slate-100">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">Tag: <span className="font-semibold text-emerald-600">{viewingAsset?.asset_tag}</span></p>
              </div>
            </div>
      ...
      {showDetailsModal && viewingAsset && (
        <div className="fixed inset-0 z-[500] md:hidden" onClick={() => setShowDetailsModal(false)}>
          ...
          >
            ...
            {/* Header */}
            <div className="flex justify-between items-start px-5 pb-3 border-b border-slate-100 shrink-0">
              <div className="min-w-0 flex-1 pr-2">
                <div className="flex items-center gap-2 flex-wrap">
                  <h3 className="text-base font-bold text-slate-900 leading-snug">{viewingAsset.name}</h3>
                  <StatusBadge status={viewingAsset.status} />
                </div>
                <div className="mt-1">
                  <span className="text-[11px] text-emerald-700 bg-emerald-50 border border-emerald-200/80 px-1.5 py-0.5 rounded font-bold font-mono tracking-tight inline-block">
