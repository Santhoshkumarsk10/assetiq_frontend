                ...
                >
                  <div className="flex justify-between items-start gap-2">
                    <div className="min-w-0 flex-1">
                      <h4 className="text-[13.5px] font-bold text-slate-900 leading-snug tracking-tight truncate">
                  ...
                  <div className="mt-2.5 pt-2 border-t border-slate-100/90 grid grid-cols-2 gap-2 text-xs">
                    <div className="min-w-0 bg-slate-50/70 border border-slate-100/90 rounded-lg px-2 py-1">
                      ...
                      <span className="font-semibold text-slate-800 text-[11.5px] truncate block leading-tight">{alloc.user?.name || "—"}</span>
                      {alloc.user?.email && (
                        <span className="text-[10px] text-slate-400 truncate block mt-0.5">{alloc.user.email}</span>
                      )}
                    </div>
                    <div className="min-w-0 bg-slate-50/70 border border-slate-100/90 rounded-lg px-2 py-1">
                      <span className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider leading-none mb-0.5">{t('allocatedBy')}</span>
                      <span className="font-semibold text-slate-800 text-[11.5px] truncate block leading-tight">{alloc.allocator?.name || "—"}</span>
                    </div>
                    <div className="min-w-0 bg-slate-50/70 border border-slate-100/90 rounded-lg px-2 py-1">
                      <span className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider leading-none mb-0.5">{t('allocatedDate')}</span>
                      <span className="font-semibold text-slate-800 text-[11.5px] truncate block leading-tight">
                        {alloc.created_at ? new Date(alloc.created_at).toLocaleDateString() : "—"}
                      </span>
                    </div>
                    <div className="min-w-0 bg-slate-50/70 border border-slate-100/90 rounded-lg px-2 py-1">
                      <span className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider leading-none mb-0.5">{t('returnedDate')}</span>
                      <span className="font-semibold text-slate-800 text-[11.5px] truncate block leading-tight">
                        {alloc.returned_at ? new Date(alloc.returned_at).toLocaleDateString() : "—"}
                      </span>
                    </div>
                  ...
                  {alloc.notes && (
                    <div className="text-xs bg-slate-50/80 border border-slate-100 rounded-lg p-2 mt-2">
                      <span className="font-bold text-slate-400 text-[9px] uppercase tracking-wider block mb-0.5">{t('notesReason')}</span>
                      <p className="text-slate-700 font-medium break-words leading-relaxed text-[11.5px]">{alloc.notes}</p>
                    </div>
                  )}
                </div>
              ))
